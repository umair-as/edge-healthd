# Edge Health Snapshot — Contract v1.1

- **Schema ID:** `edge.health.state`
- **Schema version:** `1.1`
- **Schema file:** [`schemas/edge.health.state.v1.1.json`](../schemas/edge.health.state.v1.1.json)
...
## What's new in v1.1 (honest observability)

v1.1 separates the **health** axis from the **observability** axis so a loss of
visibility can no longer masquerade as health.

- **Two new `severity` states.** In addition to `ok`/`warn`/`crit`/`unknown`, the
  `severity` enum gains:
  - `unavailable` — a monitored element could not be read this cycle (e.g. an
    unmounted path, a dead thermal sensor).
  - `stale` — a section was collected before but is now past its freshness window.

  Roll-up ranking is `unknown < ok < stale < unavailable < warn < crit`.
  `unknown` (never observed — warm-up or an absent optional dependency) stays
  **below** `ok`, so a fresh boot never alarms. `stale`/`unavailable` surface
  **above** `ok` but below `warn`/`crit`, so a real health signal — including a
  stale section's retained last-known `warn`/`crit` — still dominates the top line.

- **Per-section freshness.** Every subsystem object may carry `collected_at`
  (RFC 3339 time of its last successful collection) and `stale` (boolean). A
  never-collected section omits `collected_at` and is not `stale`.

- **Per-domain severity.** `summary.domains` exposes the severity of each of the
  seven domains (`boot`, `services`, `resources`, `time_sync`, `update`,
  `journal`, `crash`) so a single degraded/blind domain isn't hidden by the
  worst-of roll-up.

- **Honest storage/thermal availability.** Storage mounts and thermal sensors
  carry `available`. When `false` (statvfs failed / sensor unreadable or out of
  plausible range) the numeric fields (`used_pct`/`avail_mb`, `temp_c`) are
  **omitted** instead of reporting a misleading `0`, and the element contributes
  `unavailable` to `resources`.

- **`network[].speed_mbps` / `duplex` are now populated** (via the ethtool
  generic-netlink family). They remain optional and are omitted for link-down or
  ethtool-less interfaces.

### Compatibility

| Change | Kind |
|---|---|
| `severity` enum adds `unavailable`, `stale` | **changed** — consumers with an exhaustive severity switch must handle them |
| `collected_at`, `stale` on subsystem objects | additive (optional) |
| `summary.domains` | additive (optional) |
| `storage[].available`, `thermal[].available` | additive |
| `storage[].used_pct`/`avail_mb`, `thermal[].temp_c` no longer required | **relaxed** — a strict consumer must guard for their absence when `available:false` |
| `network[].speed_mbps`/`duplex` populated | behavioral (fields already existed) |

Consumers written for `1.0` keep working except where they (a) exhaustively
enumerate `severity`, or (b) assumed `used_pct`/`temp_c` are always present.
